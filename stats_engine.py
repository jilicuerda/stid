...
def generate_duel_graph(points_set, team_home_name, team_away_name, set_num):
    ...
    color_h, color_a = '#3498db', '#e67e22' # Bleu et Orange (comme votre collègue)
    
    current_score_h, current_score_a = 0, 0
    x_labels, x_colors = [], []
    pos_x = 0
    
    # On regroupe les points par "Séquence de service"
    current_server_team = points_set[0]['server_team']
    current_server_num = points_set[0]['server_num']
    points_in_sequence_h = 0
    points_in_sequence_a = 0
    
    sequences = []
    
    for p in points_set:
        # Si le serveur change, on sauvegarde la séquence et on en commence une nouvelle
        if p['server_team'] != current_server_team or p['server_num'] != current_server_num:
            sequences.append({
                'team': current_server_team, 'server': current_server_num,
                'score_h': current_score_h, 'score_a': current_score_a
            })
            current_server_team = p['server_team']
            current_server_num = p['server_num']
            
        current_score_h = p['score_home']
        current_score_a = p['score_away']
...
def dessiner_terrain(ax, team_serve_name, pos_serve, team_recv_name, pos_recv, is_home_serving):
    """Dessine un terrain avec les positions (Code de votre collègue adapté)"""
    ...
    # Coordonnées des 6 positions sur les deux demi-terrains
    ...
    # Placement des joueurs (1 à 6 correspondent aux index 0 à 5 dans notre BDD)
    positions_order = [1, 2, 3, 4, 5, 6]
    
    for i, p_num in enumerate(pos_serve[1:]): # Les 5 autres joueurs
        pos = positions_order[i+1]
        c = coords_right[pos]
        ax.text(c[0], c[1], str(p_num), fontsize=14, weight='bold', color=color_serve, ha='center')
        
    for i, p_num in enumerate(pos_recv):
...
def generate_rotation_graph(points_set, team_home, team_away):
    ...
    rot_home = first_point['rotation_home'].split('-')
    rot_away = first_point['rotation_away'].split('-')
    
    fig, ax = plt.subplots(1, 1, figsize=(10, 4))
    
    if first_point['server_team'] == team_home:
